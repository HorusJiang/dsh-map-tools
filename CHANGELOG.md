...
- **地图卡上的署名**（`本卡片由 DeepSeek Harness 插件 dsh-map-tools 生成 · GitHub ↗`），接在
  「在高德打开 ↗」**同一行的后面**。地图卡经常被截图转发，而截图里没有 README、
  没有包页——这行字是图里唯一能说明"它是谁画的"的信息。整张回合尾部卡片**只出现
  一次**（挂在最后一条路线上，不是每条路线一次），包名用正文色 + 中等字重，缩略图里
  也认得出；链接用主题链接色、带 `target="_blank" rel="noreferrer"`；没有几何
  （老日志 / 高德公交无折线）时深链为空，署名照样在。
  纯客户端渲染：不进 `presentationMeta`、不进模型上下文、不占 token，宿主半边零改动。
  过程区的工具卡**不落款**（那里会随思考反复重绘，重复刷屏没有意义）。

### Fixed

- **卡片里混用半号字号**：内联署名那一行没定字号——深链自己写死 12px，署名继承的是
  卡片外层的字号，于是同一行两截在实机截图里明显不等大。现在字号只有"行"这一个
  来源（`link` 行给 12px，深链与署名都不自带 fontSize），并且**整张路线卡统一
  12px**（原先路书预览、"展开分段指引"按钮、卡片级备注是 11.5px）。测试加了两道
  守卫：link 行内任何显式字号都必须是 12px、路线卡片区的字号集合只能是 `['12']`。
- **路线卡片区的三个主题别名宿主里根本不存在**：`--dsw-alias-bg-elevated`、
  `--dsw-alias-border`、`--dsw-alias-accent` 在运行中的 ui-theme 令牌表里查无此名
  （逐个 token 数过定义数：0），卡片底色 / 描边 / 链接色因此一直静静地落在硬编码
  fallback 上、深浅色不跟随。0.7.0 修的是配置卡里的同一批名字，路线卡片区当时漏了。
  现在改用真实令牌 `--dsw-alias-bg-layer-1` / `--dsw-alias-border-l2` /
  `--dsw-alias-link`，并把主题令牌登记表**从配置卡区扩展到路线卡片区**，另加一条
  "三个假名字不得再出现"的断言。

### Changed

- **README 补齐桌面版（DeepSeek Harness Desktop）的安装方式**：新增「方式一：桌面版插件页
  安装」——侧边栏「插件」页 → 「添加插件」→「包名或地址」填 `dsh-map-tools`（安装源可选
  默认 / npm 官方源 / 中国大陆镜像源 / 自定义地址），命令行安装降为方式二 / 方式三并分别
  标注 `web` 与 `desktop` 两个 profile 名。同时写明桌面版装进 **`desktop` profile**、
  界面写下的 `^0.7.1` 在 0.x 下升不到 0.8.x（升级要在同一处填目标版本），并把"需要
  web profile"改成"需要 **Web 客户端**（`dsh web` 与桌面版都算）"；中英双语同步。
- 记录 **DSH 0.1.7-rc.2 兼容**：DeepSeek Harness Desktop 0.1.7-rc.2 上插件页安装与回合
  尾部地图卡可用（该版本 `conversation.chat.turnTail` 仍声明为 `list`，与插件注册形状
  一致）。
...
- **分段指引只给前 12 步，反而逼模型去"补算"，由此派生出那条会占掉卡片的分段路线**。
  实测那次的模型推理原文就是 `the step list was truncated at step 12 of 16. Let me get
  the complete step list.`——它要回答"终点走哪个门、最后几步怎么走"，手里却只有前 12 步，
  于是自己去地理编码路口、再算一条分段路线来反推；那条探测把卡片占掉，用户看到的地图
  就成了另一条路线。这是本次一连串"地图和正文对不上"的**共同起因**。
  现在：≤24 步全部给出；更长则给前 16 步 + "中间省略 N 步" + **最后 6 步**
  （到达段永远在，模型没有理由再去补算）。
- **模型看不到"坐标到底是哪"，于是会把别的路口当成起点**。实测踩坑：用户问
  南艳湖联想科技港 → 蜀山森林公园，模型为补全主路线被截断的分段指引，自己去地理
  编码了"南二环路与宿松路交口"，并**拿那个坐标当了起点**，算出一条 13.3 公里的
  分段路线——它的参数里只有一串数字，模型自己、以及读过程区的人都看不出传错了。
  现在路线工具的**模型可见文本回显解析后的起终点名称**，坐标入参时标注
  "（坐标反查）"：`起终点：南二环路绿地国际创客中心 → 蜀山森林公园（坐标反查）`，
  传错起点一眼可见。
- **卡片的语义模型不知道：它只镜像本回合的路线调用**。模型习惯在回答回合里顺手算
  一段分段来核对细节，而卡片只认本回合的调用——于是卡片画的是那段核对、正文讲的
  却是上一回合的主路线。四个路线工具的 description 现在明确写出这条契约：
  卡片只镜像本回合的调用（含为核对而算的分段）；回答引用更早回合的路线，
  请在本回合用相同起终点再调用一次。
- **一轮有多条路线时"张冠李戴"：正文讲 A→B，地图画的是 C→B**。模型常在同一轮里
  发多条路线调用——先算主路线，再算若干用于核对/备选的探测（往往直接传坐标，反查
  出来的地名与用户问的毫不相干）。卡片原先固定显示**最后一条**，于是用户问
  "南艳湖联想科技港 → 蜀山森林公园"（20.4 km），地图画的却是中间那条探测
  "望江西路欣塘家园 → 蜀山森林公园"（7.4 km）。现在选择器**不再挑**，按时间顺序把
  本轮所有路线交给卡片，卡片逐条渲染（每条都有自己的地图，标注"第 k/N 条"；超过
  3 条只显示前 3 条并注明总数）。
  两种"猜主路线"的启发式都试过并否掉了，记在这里免得后人再走一遍：
  - **按用户原话匹配**：回合级累加器拿不到用户消息——`user/message` 事件没有 `turn`
    字段（官方 `input-message` 定义是用 message id 另起一个节点），所以 id 对不上、
    状态收不到文本。
  - **按收尾正文匹配**：收尾正文会引用探测路线的坐标，最长公共子串反而给探测更高分
    （实测 19 vs 9），会把主路线挑错。
...
- **已知限制：卡片只看本回合**。如果模型在回答回合里只算了分段（主路线是上一回合
  算的），卡片仍会显示那段分段——这是结构性的：链式槽位的 owner 只给当前 Turn 的
  数据，卡片读不到更早回合的路线。上面两条修复把**根因**（模型不知道坐标是哪、
  不知道卡片语义）从行为上堵住，但没有做成"跨回合记忆"。要做得先确认客户端能否
  读取更早 Turn 的 Turn 数据。
